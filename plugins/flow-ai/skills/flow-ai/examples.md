...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/pipelines" | \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/samples/search" \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/projects/search" \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/samples/12345/executions" \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/samples/12345/data" \
  --data-urlencode "count=100" | \
...
```bash
curl -s -A "flow-ai/0.7.1" -o sample.fastq.gz \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/data/67890/contents" | \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/samples/12345" | \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/projects/4567" | \
...
```bash
curl -s -A "flow-ai/0.7.1" --get "${FLOW_API_URL:-https://app.flow.bio/api}/data/67890" | \
...
HUMAN_ID=$(curl -s -A "flow-ai/0.7.1" \
...
uvx --from "flowbio==0.7.0" flowbio data upload ./counts.tsv \
  --json --no-progress
```
...
> (`pip install "flowbio>=0.7.0"`), then ask me to upload again.
...
PROJECT_ID=$(curl -s -A "flow-ai/0.7.1" \
...
An upload, not a query — it runs through the on-demand flowbio CLI, not
`curl`. Read SKILL.md §4 and the `POST /upload/multiplexed` section of
`endpoints/samples.md` for the full contract. Suppose the user says:
"upload multiplexed reads `pool_R1.fastq.gz` and `pool_R2.fastq.gz` with
my annotation sheet `annotation.xlsx`, accept any warnings".
...
A helper for the multiplexed flow (Example 20), and a **read** — it changes
no remote state, so there is **no** confirmation gate. It still runs through
the on-demand flowbio CLI (not `curl`) so auth and base-URL handling stay
uniform. Read the `GET /annotation/<sample_type>` section of
